...
<div id="templatemo_wrapper">
                		...
                		<c:forEach items="${categoryList }" var="category" varStatus="vs">
                			<c:if test="${vs.index !=0}">
                				<c:if test="${vs.index != fn:length(categories)-1 }">
                					<li>
                						<a href="${pageContext.request.contextPath }/admin/ProductServlet?op=byCid&cid=${category.cid}">${category.cname}</a>
                					</li>
                				</c:if>
                			</c:if>
                			<c:if test="${vs.index==0 }">
        ...
        <div id="content" class="float_r">
        	...
        	<c:forEach items="${hotProducts }" var="product" varStatus="vs">
        		<div class="${vs.index % 3 != 2?'product_box':'product_box no_margin_right' }">
		            <h3>${product.pname }</h3>
	            	<a href="${pageContext.request.contextPath }/admin/ProductServlet?op=findProductById&pid=${product.pid}">
	                ...
	                <a href="${pageContext.request.contextPath }/admin/ProductServlet?op=findProductById&pid=${product.pid}" class="detail"></a>
            	</div>
	...
	<script type="text/javascript">
		...
		function addCart(pid,uid){
			window.location.href="${pageContext.request.contextPath}/admin/CartServlet?op=addCart&pid="+pid+"&uid="+uid;
		}
	</script>
</body>
</html>